...
const OfferPackages = () => {
    ...
    return (
        <section id="packages" className="bg-surface-100/70">
            <div className="container-x py-16">
                ...
                <div className="mt-10 grid gap-6 auto-rows-fr sm:grid-cols-2 lg:grid-cols-4">
                    {sessionPacks.map((p, i) => (
                        <article
                            key={i}
                            className="h-full rounded-2xl border border-brand-200/50 bg-white shadow-soft p-6 flex flex-col"
                        >
                            <header>
                                <h3 className="text-lg md:text-xl font-semibold text-gray-900">{p.title}</h3>
                                <div className="mt-2 text-2xl md:text-3xl font-bold text-gray-900">{p.price}</div>
                            </header>
                    ...
                    <article className="h-full rounded-2xl border-2 border-brand-600 bg-gradient-to-b from-brand-50 to-white p-6 flex flex-col">
                        <header className="flex items-center gap-3">
                            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                                <path
                                    ...
                                    fill="none"
                                />
                                <path d="M14 10l-4 4" stroke="currentColor" className="text-brand-700" strokeWidth="1.5" />
                            </svg>
                            <h3 className="text-xl font-semibold text-gray-900">{intensive.title}</h3>
                        </header>
                        <div className="mt-2 text-3xl font-bold text-gray-900">{intensive.price}</div>
                        <p className="mt-3 text-sm text-text-secondary">{intensive.desc}</p>
                        <div className="mt-4 rounded-lg bg-brand-50 text-brand-900 border border-brand-200 p-3 text-sm">
                            {intensive.note}
                        </div>
                        <div className="mt-auto pt-6 text-xs text-gray-400">{t("packages.smallPrint")}</div>
                    </article>
                </div>
            </div>
        </section>
